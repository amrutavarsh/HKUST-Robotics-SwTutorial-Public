...
// a enum that represent the flashing sequence of bonus mode
typedef enum {
	LED_ON = 0,
	LED_OFF,
	LED_FLASH
...
Button btnSequence[3];   // an array store the sequence of button press
u32 last_switch_led_ticks = 0;  // remember the last switch LED time, when BUTTON1 is holding

// Bonus mode's variable
LED_MODE bonusState = LED_ON;
u8 index = 0;
u8 blink = 0;
u32 delay_tick = 0;
...
int main() {
	...
	u8 ledState = 0;  // variable that remember the current ledState
			...
			switch (mode) {
				case NORMAL:  // Normal mode
					...
					if (btn[BUTTON1] && btn[BUTTON2] && btn[BUTTON3]  // if all three buttons is pressed
						...
						btnSequence[0] = BUTTON1;
						btnSequence[1] = BUTTON2;
						btnSequence[2] = BUTTON3;

						// simple sorting according to the button press time
						// in case you want to know, it is a bubble sort algorithm
						for (u8 i = 0; i < 2; ++i) {
							for (u8 j = 0; j < 2 - i; ++j) {
								if (btn[btnSequence[j]] > btn[btnSequence[j+1]]) {
									Button temp = btnSequence[j];
									btnSequence[j] = btnSequence[j+1];
									btnSequence[j+1] = temp;
								}
							}
						}
						
						mode = BONUS;  // switch the mode to Bonus mode
						resetSequence();
					}
					break;
				...
				case BONUS:
					...
					if (btn[BUTTON1] && btn[BUTTON2] && btn[BUTTON3]  // if all three buttons is pressed
						...
						// initialize the array
						Button newBtnSequence[3] = {BUTTON1, BUTTON2, BUTTON3};

						// simple sorting according to the button press time
						// in case you want to know, it is a bubble sort algorithm
						for (u8 i = 0; i < 2; ++i) {
							for (u8 j = 0; j < 2 - i; ++j) {
								if (btn[newBtnSequence[j]] > btn[newBtnSequence[j+1]]) {
									Button temp = newBtnSequence[j];
									newBtnSequence[j] = newBtnSequence[j+1];
									newBtnSequence[j+1] = temp;
								}
							}
						}

						// compare if this button sequence is equal to the entering sequence
						if (btnSequence[0] == newBtnSequence[0]
							&& btnSequence[1] == newBtnSequence[1]
							&& btnSequence[2] == newBtnSequence[2]) {
							led_on(selectedLED);  // make sure the selected LED is on
							mode = NORMAL;  // switch back to Normal mode
						}
					}
					
					// There is another finite state machine in the bonus mode
					switch (bonusState) {
						case LED_ON:
							if (this_ticks - last_switch_led_ticks >= delay_tick) {
								if (index < 4)  // if the LED sequence not yet end, don't go to the constant flashing mode first
									bonusState = LED_OFF;
								else
									bonusState = LED_FLASH;
								led_on(selectedLED);
								last_switch_led_ticks = this_ticks;
							}
							break;
						...
						case LED_OFF:
							if (this_ticks - last_switch_led_ticks >= 200) {
								led_off(selectedLED);
								bonusState = LED_ON;
								blink--;
								if (blink) {  // if there are still blink
									delay_tick = 200;
								} else {  // otherwise set the blink to the next sequence
									blink = btnSequence[index] + 1;
									index++;
									delay_tick = 500;
								}
								last_switch_led_ticks = this_ticks;
						...
						case LED_FLASH:
							if (this_ticks - last_switch_led_ticks >= 300) {
								if (ledState) {
									led_off(selectedLED);
									ledState = 0;
								} else {
...
// a helper function that help reset and replay the LED flash sequence
void resetSequence(void) {
	bonusState = LED_ON;  // reset the state to default state
	led_off(selectedLED);
	last_switch_led_ticks = get_ticks();  // get the current ticks
	delay_tick = 1000;  // set the initial delay ticks to 1000ms
	blink = btnSequence[0] + 1;  // set the blink time according to the button
	index = 1;  // set the future sequence to the next button
}
